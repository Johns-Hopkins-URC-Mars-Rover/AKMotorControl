...
Servo_Params = {
        ...
        'AK70-10':{
            ...
            'V_min' : -100000,#-320000 rpm electrical speed
            'V_max' : 100000,# 320000 rpm electrical speed
            'Curr_min':-1500,#-60A is the acutal limit but set to -15A
            'Curr_max':1500,#60A is the acutal limit but set to 15A
            'T_min' : -30,#NM
            'T_max' : 30,#NM
            'Kt_TMotor' : 0.123, # from TMotor website (actually 1/Kvll)
            'Current_Factor' : 0.59,
            'Kt_actual': 0.123,
            'GEAR_RATIO': 10.0, 
            'NUM_POLE_PAIRS': 21,
...
class motorListener(can.Listener):
    ...
    def __init__(self, canman, motor):
        ...
        """
        self._canman = canman
        self.bus = can.interface.Bus('can0', interface='socketcan', bitrate=1000000)
        self.motor = motor
...
#*****************Parsing message data********************************#
    def parse_servo_message(self, data):
        ...
        pos_int = np.int32(data[0] << 8 | data[1])
        spd_int = np.int32(data[2] << 8 | data[3])
        cur_int = np.int32(data[4] << 8 | data[5])
...
class TMotorManager_servo_can():
    ...
    """
    def __init__(self, motor_type='AK70-10', motor_ID=8, max_mosfett_temp = 50, CSV_file=None, log_vars = LOG_VARIABLES):
        """
    ...
    def _send_command(self):
        ...
        if self._control_state == _TMotorManState_Servo.DUTY_CYCLE:
            ...
            print("sending velo")
            self._canman.comm_can_set_rpm(self.ID, 10)#self._command.velocity)
